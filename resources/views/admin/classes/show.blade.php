...
<div class="space-y-6">
        ...
        <div class="p-5">
            @forelse($class->sections ?? [] as $section)
                <div class="flex items-center justify-between py-3 {{ !$loop->last ? 'border-b border-gray-100' : '' }}">
                    <div class="flex items-center gap-3">
                        <div class="w-8 h-8 rounded-lg bg-[#BFECFF]/20 flex items-center justify-center">
                            <span class="text-[#CDC1FF] font-semibold text-sm">{{ $section->name }}</span>
                        </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-5">
        ...
        <form method="POST" action="{{ route('sections.store', $class) }}" class="flex gap-3">
            ...
            <button type="submit" class="px-4 py-2 bg-[#BFECFF]/200 hover:bg-primary-600 text-[#1e293b] text-sm font-medium rounded-lg transition">Add</button>
        </form>
    </div>
</div>
@endsection
