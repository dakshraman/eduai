...
export default function Index({ classes, students, classId, date, attendanceData }) {
    ...
    const loadStudents = () => {
        router.get('/attendance', { class_id: selectedClass, date: selectedDate }, { preserveState: true });
    };
                                    ...
                                    <TableBody>
                                        {students.map((student, idx) => (
                                            <TableRow key={student.id}>
                                                ...
                                                <TableCell>
                                                    <RadioGroup
                                                        value={data.attendance[student.id] || ''}
                                                        onValueChange={(val) => setAttendance(student.id, val)}
                                                        className="flex items-center justify-center gap-2"
                                                    >
                                                        {STATUSES.map((s) => (
                                                            <label key={s.value} className={`flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium cursor-pointer transition-colors ${data.attendance[student.id] === s.value ? s.color : 'bg-muted text-muted-foreground'}`}>
